...
class Controller extends Object
{
    ...
    //运行文件有脚本的扩展名
    protected $scriptExt = '.php';
    //自动CURD的权限
    protected $autoActionRight = false;
    //验证码变量名
    ...
    public function getViewFile($viewName)
    {
        ...
        {
            if(is_file($viewFile.$this->templateExt))return $viewFile.$this->templateExt;
        }

        return APP_CODE_ROOT.'views'.DIRECTORY_SEPARATOR.$this->getId().DIRECTORY_SEPARATOR.$viewName.$this->templateExt;
    }
    /**
     * 取得布局文件
     *
    ...
    public function getRunFile($view){
        return Tiny::app()->getRuntimePath().DIRECTORY_SEPARATOR.$this->getId().DIRECTORY_SEPARATOR.$view.$this->scriptExt;
    }
    /**
     * 渲染文件视图
     *
